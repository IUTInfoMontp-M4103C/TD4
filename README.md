...
Dans ce TD, nous allons réinvestir le travail des trois premiers TD pour créer une version web du jeu du taquin. L'environnement html-css est déjà opérationnel, et vous avez dans le dossier img plusieurs dossiers contenant toutes les images nécessaires pour le jeu.

Le travail à accomplir : créer les fichiers JavaScript qui permettent de jouer. 

Nous pouvons donner une version strictement fonctionnelle du jeu, ou alors nous pouvons organiser notre code en classes.
...
Cette classe sert à instancier des objets Emplacement qui sont concrètement notre représentation des emplacements du taquin. Attention, un emplacement est fixe. Il y en a donc 16 dans un taquin. Chaque emplacement est caractérisé par un numéro de ligne (entre 0 et 3), un numéro de colonne (idem), un thème (qui est une chaîne de caractères permettant d)
